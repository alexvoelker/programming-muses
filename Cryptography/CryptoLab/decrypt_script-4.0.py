...
def password_checker():
    ...
    while len(WORD_LIST) > 0:
        ...
        key_check_msg = f"[INFO] {datetime.datetime.now().strftime('%X')} | Current Key Checked [ " \
            f"{WRD_LST_INDEX / WRD_LST_TOTAL * 100:.2f}% | {WRD_LST_INDEX} / {WRD_LST_TOTAL} ]: \t{current_password}"
        with open(f"{PATH}{LOG_FILE_NAME}", 'a') as log_file:
